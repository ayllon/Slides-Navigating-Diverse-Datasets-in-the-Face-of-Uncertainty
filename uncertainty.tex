...
\setbeamertemplate{footline}{%
  \begin{beamercolorbox}[wd=\textwidth, sep=3ex]{footline}%
    ...
    \hfill%
    \usebeamertemplate*{frame numbering}\vskip-4pt
  \end{beamercolorbox}%
...
\begin{itemize}
    ...
    \item 5.688 papers in 2017 (242 accepted), 2.468 in 2022 (89 accepted)
\end{itemize}
}
\end{frame}
...
\note{
    "A distributed in-situ analysis method for large-scale scientific data" not explicitly interactive, but within limits
}
\end{frame}
...
\section{\PresQ}
\note{
    Be explicit this is the main contribution (state-of-the-art is done).
    Add to each slide the section (breadcrumb).
}
...
\note{
    This *needs* to be satisfied. The contribution is the proof.
    Follow the Thesis schema (before PresQ section). Or EDD after these slides.
...
\note{
    This is adapted from IND, not a contribution.
}
\end{frame}
...
\note{
    Another option is Top-Down search (optimistic): we assume $n$ 1-EDDs are derived from a single $n$-EDD.
    \bigskip
    But we risk being too optimistic and needing to traverse the full search space from top to bottom.
    \bigskip
    We can alternate between bottom-up (pessimistic) and top-down (optimistic).
}
\end{frame}
...
\note{
A hypergraph is a generalization of a graph. In a graph, an edge connects two nodes. In
a k-uniform hypergraph, an edge connects k nodes.
}
\end{frame}
...
\note{
 ...
 TODO: Change color node I mention
}
\end{frame}
...
\begin{frame}{Clique Recovery: Missing Edges}
    ...
    \end{figure}
    \note{Remove timeouts, separate and make clearer}
\end{frame}
...
\note{
Removed and added purely random. With actual data $\Lambda$ is more robust.
}
\end{frame}
...
\note{
Tables 5.6 and 5.7 contain more detailed results for this experiment.
Get rid of the figure and verbally explain.
}
\end{frame}
...
\begin{enumerate}
    ...
    \item Approach 2
    \begin{itemize}
        \item Can not use the full set to do the test (problem for small samples)
...
\note{
    \begin{itemize}
        ...
        \item The plot shows empirical significance level (95\%)
...
\note{
Codebooks for a subset of the parameters: how each neuron is sensitive to a given feature. Darker is more.
}
\end{frame}
...
\begin{frame}{Future Work}
    \begin{itemize}
        ...
        \item A Bayesian EDD finding algorithm (i.e., prior support, partial EDD~\footcite{soriano2015bayesian}).
...
\begin{itemize}
    ...
    \item Bayesian: can check local null hypothesis testing, so we can accept a candidate with only partial support
    \item Privacy preserving: linking attacks
    \begin{itemize}
        \item  Experiment with Open University Learning Analytics Dataset shows an "attribute disclosure": the attackers improve their knowledge of a set of attributes for a given individual. i.e. young female from North Western, most likely deprived (low income, low employment, etc.).
    \end{itemize}
    \item TODO: Add backup slide privacy-preserving
    \item TODO Contributions last thing?
    \item Add one more slide for photos and stop there
\end{itemize}
}
\end{frame}
