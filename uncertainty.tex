...
\begin{frame}{Definition of the Search Space}
    ...
    \begin{block}{Satisfiability}
         Let $I_1$, $I_2$ be two candidate INDs such that $I_1 \prec I_2$.
         \begin{enumerate}
...
\begin{figure}
    ...
    \alert{Can we generalize IND to uncertain, multidimensional sets of attributes?}
\end{figure}
\note{
    Lattice!
...
\note{
\begin{itemize}
    \item Another option is Top-Down search (optimistic): we assume $n$ 1-EDDs are derived from a single $n$-EDD.
    \item But we risk being too optimistic and needing to traverse the full search space from top to bottom.
    \item We can alternate between bottom-up (pessimistic) and top-down (optimistic).
    ...
    \item BACKUP: Find2 Complexity
    $\Theta(|V|^4 · |C|^2)$, where $|C|$ is the set of cliques.
\end{itemize}
}
\end{frame}
...
\begin{frame}{Internal Validity}
    \begin{alertblock}{Significance of the Results}
        ...
        \begin{itemize}
            ...
            \item The uncertainties are reported explicitly using either 95\% confidence intervals or quartiles.
        \end{itemize}
    \end{alertblock}
\end{frame}
